...
class KeyEventHandler : public MessageHandler {
public:
	KeyEventHandler( void );
	virtual ~KeyEventHandler( void );

	void send( Connector *conn, const std::set< ::util::key::type > &keys );

	typedef boost::function<void ( KeyEventData *data )> KeyEventCallback;
	void onKeyEvent( const KeyEventCallback &callback );
	virtual void process( util::Buffer *msg );

private:
	KeyEventCallback _onKeyEvent;
};
