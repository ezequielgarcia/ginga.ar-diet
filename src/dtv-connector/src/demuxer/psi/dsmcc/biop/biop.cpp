...
void show( const Tap &tap ) {
	printf( "[biop::Tap] id=%04x, use=%04x, associationTag=%04x, sLen=%d\n",
		tap.id, tap.use, tap.assocTag, tap.selector.length() );
}

SSIZE_T parseTaps( BYTE *data, std::vector<Tap> &taps ) {
	SSIZE_T offset=0;
	...
	for (BYTE t=0; t<tCount; t++) {
		...
		if (sLen) {
			tap.selector.copy( (char *)(data+offset), sLen );
		}
		offset += sLen;
...
void parseModuleInfo( const util::Buffer &moduleInfoBytes, ModuleInfo &info ) {
	BYTE *data = (BYTE *)moduleInfoBytes.buffer();
	SSIZE_T offset = 0;
	...
	info.userInfo.copy( (char *)(data+offset), usrInfoLen );
	offset += usrInfoLen;
...
void show( const ModuleInfo &info ) {
	printf( "[biop::ModuleInfo] moduleTimeOut=%08lx, blockTimeOut=%08lx, minBlockTime=%08lx, taps=%d, userInfo=%d\n",
