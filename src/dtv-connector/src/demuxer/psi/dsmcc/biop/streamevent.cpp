...
bool StreamEvent::parseObjectInfo( Module *module, SSIZE_T offset, WORD objInfoLen ) {
	//	Read all object info
	_buf.resize( objInfoLen );
	if (module->read( offset, (BYTE *)_buf.buffer(), objInfoLen ) != objInfoLen) {
		printf( "[biop::StreamEvent] Cannot read ObjectInfo\n" );
		return false;
	}
	SSIZE_T off = 0;
	BYTE *ptr   = (BYTE *)_buf.buffer();
...
bool StreamEvent::parseBody( ResourceManager */*resMgr*/, Module *module, SSIZE_T offset, DWORD bodyLen ) {
	//	Read all body
	_buf.resize( bodyLen );
	if (module->read( offset, (BYTE *)_buf.buffer(), bodyLen ) != bodyLen) {
		printf( "[biop::StreamEvent] Warning, cannot read object body\n" );
		return false;
	}
	...
	if (eventIDs != _events.size()) {
		printf( "[biop::StreamEvent] Warning, events names and ids not deffers\n" );
		return false;
	}
...
void StreamEvent::process( Object *parent, const std::string &objName, std::list<Object *> & /*unprocessed*/ ) {
    ...
    printf( "[dsmcc::StreamEvent] Object name %s processed\n", getPath(parent,objName).c_str() );
    //  Do nothing	
}
...
void StreamEvent::getEvents( std::vector<dsmcc::Event *> &events ) {
	...
	BOOST_FOREACH( const Tap &tap, _taps ) {
		if (tap.use == BIOP_TAP_STR_EVENT_USE) {
			events.push_back( new dsmcc::Event( name(), tap.assocTag, _events ) );			
		}
	}
}
