...
void StreamEventDemuxer::onSection( BYTE *section, SIZE_T /*len*/ ) {
    ...
    switch (descTag) {
		...
		case DSMCC_SE_GINGA_EDITCMD:
			...
			break;
        default:
			printf( "[DSMCCDemuxer] Warning: Stream event %x not handled\n", descTag );
    };
}

Version StreamEventDemuxer::updateVersion( Version ver, BYTE *section, bool apply ) {
	StreamEventDemuxer::Subtable sub;	
...
void StreamEventDemuxer::parseSEStreamEvent( BYTE *section ) {
	...
	if (it != _eventFilters.end()) {
		util::Buffer buf( (const char *)section, descLen+2, false );
		(*it).second( buf );
	}
}
