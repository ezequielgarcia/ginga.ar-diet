...
bool DataCarouselHelper::collectTags( Service *srv, bool (*cmp)( BYTE type ) ) {
	...
	BOOST_FOREACH( const Pmt::ElementaryInfo &elem, srv->elements() ) {
		if ((*cmp)(elem.streamType)) {
			...
			it = elem.descriptors.find( desc::ident::association_tag );
			if (it != elem.descriptors.end()) {
				desc::AssociationTagDescriptor assDesc = (*it).second.get<desc::AssociationTagDescriptor>();
				find |= addTag( tags, assDesc.tag, elem.pid );
			}

			//	Find stream_identifier_descriptor
			it = elem.descriptors.find( desc::ident::stream_identifier );
			if (it != elem.descriptors.end()) {
				BYTE tag = (*it).second.get<BYTE>();
...
ID DataCarouselHelper::findTag( BYTE tag ) const {
	ID pid=TS_PID_NULL;
	printf( "[DataCarouselHelper] Find Tag: tag=%02x\n", tag );
...
bool DataCarouselHelper::addTag( Tags &tags, BYTE tag, ID pid ) {
	...
	if (findPID == TS_PID_NULL) {
		printf( "[DataCarouselHelper] Add tag: tag=%02x, pid=%04x\n", tag, pid );
...
ID DataCarouselHelper::findTag( const Tags &tags, BYTE tag ) const {
	...
	return (it != tags.end()) ? (*it).second : TS_PID_NULL;
}

ID DataCarouselHelper::findTag( ID serviceID, BYTE tag ) const {
	ID pid=TS_PID_NULL;
	printf( "[DataCarouselHelper] Find Tag: service=%04x, tag=%02x\n", serviceID, tag );
