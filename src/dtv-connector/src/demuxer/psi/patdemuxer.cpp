...
void PATDemuxer::onSection( BYTE *section, SIZE_T len ) {
	//  Parse the programs
	for (SSIZE_T x=PSI_PAYLOAD_OFFSET; x<len; x+=4) {
...
void PATDemuxer::onComplete( BYTE *section, SIZE_T /*len*/ ) {
	//	Sanity check 
	check();
	
	//  PSI_CUSTOM = Transport Stream ID
	Pat *pat = new Pat( _version, PSI_EXTENSION(section), _nitPid, _programs );
	_onParsed( pat );
}

Version PATDemuxer::updateVersion( Version ver, BYTE */*section*/, bool apply ) {
...
void PATDemuxer::check() {
	...
	while (it != _programs.end()) {
		...
		if (erase) {
			if (prog.program != NETWORK_ID_PROGRAM) {
				printf( "[PATDemuxer] Warning, program removed: id=%04x, pid=%04x\n", prog.program, prog.pid );
			}
