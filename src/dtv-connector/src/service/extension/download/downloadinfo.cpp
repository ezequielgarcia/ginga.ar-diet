...
DownloadInfo::DownloadInfo( util::BYTE tag, util::DWORD downloadID, bool isCompulsory )
{
	_pid          = TS_PID_NULL;	
...
bool DownloadInfo::isComplete() const {
	return _filesCount == static_cast<int>(_files.size());
}
...
void DownloadInfo::files( int count ) {
	_filesCount = count;
}

void DownloadInfo::addFile( const std::string &file ) {
