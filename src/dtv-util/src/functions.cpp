...
void removeSign(string* str){
	str->erase(0,1);
}

bool isPositionProperty(string propName){
	return (propName == "size" || propName == "bounds" || propName == "top" ||
		propName == "left" || propName == "bottom" ||
		propName == "right" || propName == "width" ||
		propName == "height" || propName == "location");
}
...
bool compareSets(const std::set<int> *set1, const std::set<int> *set2) {
	...
	std::set<int>::const_iterator i = set1->begin();
...
DWORD crc_calc( DWORD crc, const BYTE *buffer, size_t len ) {
	static DWORD const crctab[256] = {
