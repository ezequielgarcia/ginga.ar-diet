...
namespace layout {
	...
	bool LayoutRegion::setBottom(double newBottom, bool isPercentual) {
		...
		if (parent != NULL) {
		    if (isPercentual)
                tBottom = parent->getHeightInPixels() - (newBottom / 100) * parent->getHeightInPixels();
            else tBottom = parent->getHeightInPixels() - newBottom;
            bottomPercentual = false;
	...
	bool LayoutRegion::setTargetBottom(double newBottom, bool isPercentual) {
		double tBottom;
        double parent_height = parent->getHeightInPixels();
		...
		if (isPercentual)
		    tBottom = parent_height - (newBottom / 100) * parent->getHeightInPixels();
		else tBottom = parent_height - newBottom;
	...
	bool LayoutRegion::setHeight(double newHeight, bool isPercentual) {
		...
		if (parent != NULL) {
		    ...
		    if (isPercentual)
                tHeight = (newHeight / 100) * parent->getHeightInPixels();
            else tHeight = newHeight;
            heightPercentual = false;
	...
	bool LayoutRegion::setTargetHeight(double newHeight, bool isPercentual) {
		...
		/*if (!isnan(bottom))
            setBottom(parent->getHeightInPixels() - getBottom(), false);
*/
		return true;
	}
	...
	bool LayoutRegion::setTargetLeft(double newLeft, bool isPercentual) {
		...
		if (isPercentual) {
			tLeft = (newLeft / 100) * parent->getWidthInPixels();
		} else {
			tLeft = newLeft;
		}
	...
	bool LayoutRegion::setRight(double newRight, bool isPercentual) {
		...
		if (parent != NULL) {
			if (isPercentual)
				tRight = parent->getWidthInPixels() - (newRight / 100) * parent->getWidthInPixels();
			else tRight = parent->getWidthInPixels() - newRight;
			rightPercentual = false;
			if ((getLeftInPixels() + getWidthInPixels()) >
				     parent->getWidthInPixels()) {
	...
	bool LayoutRegion::setTargetRight(double newRight, bool isPercentual) {
		double tRight;
		double parent_width =parent->getWidthInPixels();
		...
		if (isPercentual) {
			tRight = (newRight / 100) * parent_width;
		//	if (isNaN(width))
				tRight = parent_width - tRight;
		} else {
			tRight = parent_width - newRight;
		}
		right = tRight;
		rightPercentual = false;
	...
	bool LayoutRegion::setTargetTop(double newTop, bool isPercentual) {
		...
		if (isPercentual) {
			tTop = (newTop / 100) * parent->getHeightInPixels();
    ...
    bool LayoutRegion::setWidth(double newWidth, bool isPercentual) {
		...
		if (parent != NULL) {
            if (isPercentual)
                tWidth = (newWidth / 100) * parent->getWidthInPixels();
            else tWidth = newWidth;
            widthPercentual = false;
	...
	bool LayoutRegion::setTargetWidth(double newWidth, bool isPercentual) {
		...
		if (!isnan(right))
			setRight(parent->getRightInPixels() - getRight(), false);
