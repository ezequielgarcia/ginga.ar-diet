...
namespace layout {
	RegionBase::RegionBase(string id, IDeviceLayout* deviceLayout) : Base(id) {
		...
		this->regions           = new map<string, LayoutRegion*>;
		this->device            = "";
		this->deviceClass       = -1;
	...
	void RegionBase::createDeviceRegion() {
		...
		if (deviceProperty != NULL) {
			...
			cout << "RegionBase::createDeviceRegion '" << deviceClass << "'";
			cout << " Warning! Can't find device properties" << endl;
...
/*
		cout << endl << endl;
		cout << "RegionBase::createDeviceRegion '" << deviceClass << "'";
		cout << " and mapId '" << outputMapRegionId << "'" << endl;
		cout << endl << endl;*/
	...
	bool RegionBase::addRegion(LayoutRegion* region) {
		...
		regId = region->getId();
		/*cout << "RegionBase::addRegion Add region '" << regId;
		cout << "'" << endl;*/

		i = regions->find(regId);
		if (i != regions->end()) {
			cout << "RegionBase::addRegion Warning! Trying to add a region ";
			cout << "with id '" << regId << "' twice" << endl;
			return false;
