...
namespace connectors {
	...
	void CompoundCondition::addConditionExpression(
		...
		if (condition == NULL) {
			wclog << "CompoundCondition::addConditionExpression ";
			wclog << "Warning! Trying to add a NULL condition." << endl;
		} else {
	...
	vector<Role*> *CompoundCondition::getRoles() {
		...
		for (i = 0; i < size; i++) {
			...
			if (condition == NULL) {
				wclog << "CompoundCondition::getRoles ";
				wclog << "Warning! condition = NULL." << endl;
			}
