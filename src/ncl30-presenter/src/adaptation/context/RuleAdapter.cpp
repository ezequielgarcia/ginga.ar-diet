...
#include "../../../include/adaptation/context/RuleAdapter.h"
...
namespace context {
	...
	RuleAdapter::~RuleAdapter() {
		PresentationContext::getInstance()->removeObserver(this);
...
*/
	...
	bool RuleAdapter::evaluateSimpleRule(SimpleRule* rule) {
		...
		attributeValue = PresentationContext::getInstance()->
			    getPropertyValue(attribute);

		ruleValue = rule->getValue();

		cout << "RuleAdapter::evaluateSimpleRule '";
		cout << attribute << "' == '" << attributeValue;
		cout << "' considering rule value == '" << ruleValue << "'";
		cout << endl;
		...
		op = rule->getOperator();
		switch (op) {
			case brutil::Comparator::CMP_EQ:
				if (attributeValue == "" && ruleValue == "") {
					return true;
				} else if (attributeValue == "") {
					return false;
				} else {
					return brutil::Comparator::evaluate(attributeValue, ruleValue, op);
				}

			case brutil::Comparator::CMP_NE:
				if (attributeValue == "" && ruleValue == "") {
					return false;
				} else if (attributeValue == "") {
					return true;
