...
namespace lua {
	...
	void LuaPlayerAdapter::setCurrentEvent(FormatterEvent* event) {
		...
		if (preparedEvents->count(event->getId()) != 0 &&
			...
			currentEvent = event;
                        ApplicationExecutionObject* aeo = dynamic_cast<ApplicationExecutionObject*>(object);
                        if (aeo != NULL) {
                                aeo->setCurrentEvent(currentEvent);
                        } else {
                                std::cout << __FILE__ << __LINE__ << ": Esto no es un ApplicationExecutionObject*" << std::endl;
                        }
