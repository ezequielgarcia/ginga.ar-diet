...
namespace adapters {
	...
	void PlayerAdapterManager::readConfigFiles() {
		...
		fisMime.open(
				CONFIG_PREFIX_PATH "/etc/ginga/ncl-presenter/config/formatter/player/mimedefs.ini",
				ifstream::in);
		...
		fisCtrl.open(CONFIG_PREFIX_PATH "/etc/ginga/ncl-presenter/config/formatter/player/ctrldefs.ini");
		if (!fisCtrl.is_open()) {
