	...
	void PlainTxtPlayerAdapter::createPlayer() {
		...
		if (fileExists(mrl)) {
			...
			if (descriptor != NULL) {
				...
				paramValue = trim(descriptor->getParameterValue("fontFamily"));
				if (paramValue !="")
					player->setPropertyValue("fontFamily", paramValue);
				else{
					paramValue = trim(descriptor->getParameterValue("fontUri"));
					if (paramValue == "") {
						paramValue = PREFIX_PATH "/player/fonts/vera.ttf";
					}
				...
				paramValue = trim(descriptor->getParameterValue("fontVariant"));
				if (paramValue !="")
					player->setPropertyValue("fontVariant", paramValue);

				paramValue = trim(descriptor->getParameterValue("fontWeight"));
				if (paramValue != "")
					player->setPropertyValue("fontWeight", paramValue);

				paramValue = trim(descriptor->getParameterValue("fontStyle"));
				if (paramValue != "")
					player->setPropertyValue("fontStyle", paramValue);
			...
			cout << "Error: file " << mrl << " does not exist" << endl;
	}
}
}
}
