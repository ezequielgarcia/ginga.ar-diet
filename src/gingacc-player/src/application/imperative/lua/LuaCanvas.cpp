...
#define REFFRAME          (-4)
#define REFARC            (-5)
#define FONTDIR   PREFIX_PATH "/player/fonts/"
...
static int l_attrColor (lua_State* L)
{
	...
	if (canvas->color->getR() == 0xff &&
		canvas->color->getG() == 0x00 &&
		canvas->color->getB() == 0xff &&
		canvas->color->getAlpha() == 0xff) {

		canvas->color->setColor(0,0,0,0);
	}
...
static int l_attrFont (lua_State* L)
{
	...
	if ( strcmp(canvas->font.style, "bold-italic") == 0)
		strncpy(canvas->font.style, "BoldItalic", 20);
	else if (strcmp(canvas->font.style, "bold") == 0 || strcmp(canvas->font.style, "italic") == 0
		  || strcmp(canvas->font.style, "normal") == 0)
		canvas->font.style[0] = toupper(canvas->font.style[0]);
	else
		cout << "Error: The style " << canvas->font.style << " in attrFont is not valid" << endl;

	char path[255];
	path[0] = '\0';
	strncat(path, FONTDIR, 100);
	strncat(path, canvas->font.face, 20);
	if ( strcmp(canvas->font.style, "Bold") == 0 || strcmp(canvas->font.style, "Italic") == 0 
	      || strcmp(canvas->font.style, "BoldItalic") == 0){
		strncat(path, "-", 2);
	...
	if (!boost::filesystem::exists(path)){
		...
		path[0] = '\0';
		strncat(path, PREFIX_PATH "/player/fonts/vera.ttf", 255);
	}
...
static int l_drawEllipse (lua_State* L)
{
	...
	if (lua_equal(L, 2, -1)) {
		...
		lua_rawgeti(L, LUA_ENVIRONINDEX, REFARC);
		if (lua_equal(L, 2, -1)) {
			fill = false;
		}else{
			return luaL_argerror(L, 2, "invalid mode");
...
static int l_pixel (lua_State* L){
	...
	if (lua_gettop(L) == 7) {
		...
		// TODO: teste limites de r,g,b,a
		canvas->color = new Color(luaL_checkint(L, 4),
		                          luaL_checkint(L, 5),
	                                  luaL_checkint(L, 6),
	                                  luaL_checkint(L, 7));
		canvas->sfc->setColor(canvas->color);
		canvas->sfc->pixel(luaL_checkint(L, 2), luaL_checkint(L, 3));
		return 0;
	}
...
LUALIB_API int luaopen_canvas (lua_State* L)
{
	...
	lua_pushstring(L, "arc");                        // [ ... | 3 ]
	lua_rawseti(L, LUA_ENVIRONINDEX, REFARC);        // [ ... |  ]
