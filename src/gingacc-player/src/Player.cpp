...
namespace player {
		...
		if (code == PL_NOTIFY_NCLEDIT) {
			...
			if (code == PL_NOTIFY_STOP) {
				presented = true;
                        ...
                        std::set<IPlayerListener*> notifiables;
                        notifiables.insert(listeners->begin(), listeners->end());

                        for (i = notifiables.begin(); i != notifiables.end(); ++i) {
                                if ((*i) != NULL) {
					(*i)->updateStatus(code, parameter, type);
				}
                        }
	...
	void Player::stop() {
		this->status = STOP;
		_DFBSurfaceMutexSingleton.instance().copyToSD(); //Para actualizar cuando termina el Nominal Event Monitor
	}
