...
namespace core {
namespace player {
	class MainAVPlayer : public Player {
		...
		public:
			MainAVPlayer(string mrl);
			virtual ~MainAVPlayer();

		public:
			virtual void setPropertyValue(string name, string value, double duration=-1, double by=-1);
			void setVoutWindow(float x, float y, float w, float h);
	};
}
}
}
