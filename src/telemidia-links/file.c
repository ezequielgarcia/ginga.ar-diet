...
int allowedPath(const char *path)
{
	static const char *cage_ramdisk  = GINGA_PATH_CAGE;
	static const char *cage_pendrive = GINGA_PENDRIVE_CAGE;


	char *real_path = realpath(path, NULL);
	FILE * file;
	if ( file = fopen(real_path, "r")){
        fclose(file);
    }else{
		free(real_path);
	    return 1;
	...
	int res_ramdisk  = strncmp(real_path, cage_ramdisk, strlen(cage_ramdisk)) == 0;
	int res_pendrive = strncmp(real_path, cage_pendrive, strlen(cage_pendrive)) == 0;

	fprintf(stderr, "Pendrive cage: %i\n", res_pendrive);
	fprintf(stderr, "Ramdisk  cage: %i\n", res_ramdisk);

	free(real_path);

	return res_ramdisk || res_pendrive;
}
