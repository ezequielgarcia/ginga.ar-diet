...
int list_item_delete(struct dialog_data *dlg,struct dialog_item_data *useless)
{
	...
	if ((item->type)&1)   /* folder */
	{
		...
		msg_box(
			term,   /* terminal */
			getml(txt,narez,NULL),  /* blocks to free */
			TEXT(ld->delete_dialog_title),  /* title */
			AL_CENTER|AL_EXTD_TEXT,  /* alignment */
