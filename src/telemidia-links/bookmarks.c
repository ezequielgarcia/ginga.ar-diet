...
void *bookmark_default_value(struct session *ses, unsigned char type)
{
	...
	if (get_current_url(ses,txt,MAX_STR_LEN))
	{
		if (ses->screen->f_data)
		{
			...
			zelena->url=links_convert_string(ct,txt,strlen(txt),NULL);
	...
	if (get_current_title(ses,txt,MAX_STR_LEN))  /* ses->screen->f_data must exist here */
	{
		...
		ct=get_translation_table(ses->term->spec->charset,bookmark_ld.codepage);
		zelena->title=links_convert_string(ct,txt,strlen(txt),NULL);
...
void bookmark_edit_done(void *data)
{
	...
	if (item->title)
	{
		...
		item->title=links_convert_string(ct,title,strlen(title),NULL);
	...
	if (item->url)
	{
		...
		mem_free(item->url);
		ct=get_translation_table(s->dlg->win->term->spec->charset,bookmark_ld.codepage);
		item->url=links_convert_string(ct,url,strlen(url),NULL);
		clr_white(item->url);
	}
...
void bookmark_edit_item(struct dialog_data *dlg,void *data,void (*ok_fn)(struct dialog_data *,void * ,void *, struct list_description *),void * ok_arg, unsigned char dlg_title)
{
...

	{
		...
		txt=links_convert_string(ct,item->title,strlen(item->title),NULL);
		clr_white(txt);
		safe_strncpy(title,txt,MAX_STR_LEN);
		mem_free(txt);

		txt=links_convert_string(ct,item->url,strlen(item->url),NULL);
		clr_white(txt);
...
unsigned char *bookmark_type_item(struct terminal *term, void *data, int x)
{
	...
	txt1=links_convert_string(table,txt,strlen(txt),NULL);
	clr_white(txt1);
	mem_free(txt);
	return txt1;
}
...
void add_bookmark(unsigned char *title, unsigned char *url, int depth)
{
...

	{
		...
		b->title=links_convert_string(ct,title,strlen(title),dop);
		clr_white(b->title);
	}
	...
	if (url)
	{
		...
		dop->plain=1;
		ct=get_translation_table(bookmarks_codepage,bookmark_ld.codepage);
		b->url=links_convert_string(ct,url,strlen(url),dop);
		clr_white(b->url);
		dop->plain=0;
...
void create_initial_bookmarks(void)
{
	...
	add_bookmark("�esky",NULL,1);
	add_bookmark("Kalibra�n� procedura","http://atrey.karlin.mff.cuni.cz/~clock/twibright/links/kalibrace.html",2);
	add_bookmark("Links: dom�c� str�nka","http://atrey.karlin.mff.cuni.cz/~clock/twibright/links/index_cz.html",2);
	add_bookmark("Manu�l k Linksu","http://links.sourceforge.net/docs/manual-0.90-en/",2);*/
}
...
void save_bookmarks(void)
{
	...
	foreach(b,bookmarks)
	{
		...
		{
			unsigned char *txt, *txt1;
			txt=links_convert_string(ct,b->title,strlen(b->title),NULL);
			clr_white(txt);
		...
		else
		{
			...
			txt1=links_convert_string(ct,b->title,strlen(b->title),NULL);
			clr_white(txt1);
			txt2=links_convert_string(ct,b->url,strlen(b->url),NULL);
			clr_white(txt2);
