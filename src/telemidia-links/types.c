...
void assoc_edit_done(void *data)
{
	...
	txt=links_convert_string(table,label,strlen(label),NULL);
	mem_free(item->label); item->label=txt;

	txt=links_convert_string(table,ct,strlen(ct),NULL);
	mem_free(item->ct); item->ct=txt;

	txt=links_convert_string(table,prog,strlen(prog),NULL);
...
void ext_edit_done(void *data)
{
	...
	txt=links_convert_string(table,ext,strlen(ext),NULL);
