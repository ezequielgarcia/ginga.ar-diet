...
void dither_ARGB(unsigned short *, struct bitmap *, int *);   /* DITHER_TEMPLATE */
void round_ARGB(unsigned short *, struct bitmap *);           /* ROUND_TEMPLATE */
...
#define SAVE_CODE *outp=bt>>16;\
	...
	outp[3]=0xff;\
	outp+=4;
DITHER_TEMPLATE(dither_ARGB)
ROUND_TEMPLATE(round_ARGB)
#undef SAVE_CODE
#undef SKIP_CODE 
...
{
	switch(depth){
		case 4: //ARGB TODO: Verificar que este bien obtenido el depth en directfb_fb_init_driver
		/* 24bpp, 4Bpp 
		...
		dither_fn_internal=dither_ARGB;
		round_fn=round_ARGB;
		break;
