...
void* block_new_item(void* ignore);
void block_delete_item(void* data);
...
void block_edit_done(void *data)
{
	...
	txt=links_convert_string(table,url,strlen(url),NULL);
...
extern struct list *next_in_tree(struct list_description *ld, struct list *item);
