...
int auth_ok(struct dialog_data *dlg, struct dialog_item_data *item)
{
	...
	if (rq) {
		...
		uid = links_convert_string(ct, a->uid, strlen(a->uid), NULL);
		passwd = links_convert_string(ct, a->passwd, strlen(a->passwd), NULL);
...
int auth_window(struct object_request *rq, unsigned char *realm)
{
	...
	urealm = links_convert_string(ct, realm, strlen(realm), NULL);
