...
struct session* sessionEventHandler = NULL;
...
void setCoord(double cx, double cy, double cw, double ch) {
	x = cx;
	y = cy;
	w = cw;
	h = ch;
...
int FOCUSED_OPACITY    = 0xFF;
int UNFOCUSED_OPACITY  = 0xFF;
...
void setBrowserDFB(void* interface) {
	dfb = (IDirectFB*)interface;
}

void browserSetAlpha(int alpha, struct session* ses) {
	FOCUSED_OPACITY    = alpha;
	UNFOCUSED_OPACITY  = alpha;
	browserRequestFocus(ses);
}
...
void setGhostBrowser(struct session* ses) {
	...
	if (ses != NULL) {
		...
		data = gd->driver_data;
		data->ghostWindow = 1;
		data->window->MoveTo(data->window, 4000, 4000);
	}
}
...
void browserSetFlipWindow(struct session* ses, void* flipWindow) {
	...
	if (ses != NULL) {
		...
		data = gd->driver_data;
		data->flipWindow = (IDirectFBWindow*)flipWindow;
	}
}
...
void browserRequestFocus(struct session* ses) {
	...
	data = gd->driver_data;
	data->window->RequestFocus(data->window);
	data->window->RaiseToTop(data->window);
#endif
}
...
/* changing: begin */
void browserHide(struct session* someSes) {
  struct graphics_device* gd;
  DFBDeviceData* data;
  ...
  if (someSes != NULL) {
	  ...
	  data->flipWindow = 0;
	  gd->keyboard_handler(gd, KBD_HOME, 0);
	  data->window->SetOpacity(data->window, 0x00);
  }
}

void browserShow(struct session* someSes) {
  struct graphics_device* gd;
  DFBDeviceData *data;
...
void browserSetFocusHandler(int isHandler, struct session* someSes) {
	if (isHandler) {
		sessionEventHandler = someSes;
		browserRequestFocus(someSes);
	} else {
...
void browserReceiveEvent(struct session* ses, void* event) {
	if (events) {
		events->PostEvent(events, (DFBEvent*)event);
	}
}
...
void* browserGetSurface(struct session* someSes) {
  ...
  if (someSes == NULL || someSes->term == NULL) {
	  return NULL;
  }
  ...
  data->window->GetSurface(data->window, &s);
  return (void*)s;
#endif
  return NULL;
}
...
void* getDFBWindow(struct session* someSes) {
  ...
  data = gd->driver_data;
  return (void*)(data->window);
#endif
  return NULL;
}

void closeTerminal(struct terminal * term) {
#ifdef G
  struct graphics_device *gd;
  gd = term->dev;
...
struct session* usedSession = NULL;

void verifyPos(struct session* someSes) {
#ifdef G
	struct graphics_device *gd;
	gd = someSes->term->dev;
	...
	int posX;
	int posY;
	data->window->GetPosition (data->window, &posX, &posY);
	if (posX == 4000 && posY == 4000) {
		usedSession = someSes;
		return;
	}
#endif
...
struct session* getUsedSession() {
	struct session* someSes;
	usedSession = NULL;
	foreach(someSes, sessions) verifyPos(someSes);
	return usedSession;
}
...
void browserResizeCoord(
	...
	data->window->Resize (data->window, cw, ch);
	if (data->ghostWindow) {
		data->window->MoveTo(data->window, 4000, 4000);
	} else {
		data->window->MoveTo (data->window, cx, cy);
	}
	data->window->RaiseToTop(data->window);
#endif
}
...
directfb_flip_surface (void *pointer)
{
  ...
  if (data->flipWindow) {
	  data->flipWindow->GetSurface(data->flipWindow, &dfSurface);
	  dfSurface->StretchBlit(dfSurface, data->surface, NULL, NULL);
	  dfSurface->Flip(dfSurface, 0, 0);
  }
        ...
        case DWET_KEYDOWN:
          {
        	...
        	if (sessionEventHandler != NULL &&
        			data == sessionEventHandler->term->dev->driver_data) {
...
directfb_translate_key (DFBWindowEvent *event, int *key, int *flag)
{
  ...
  *flag = 0;
  ...
  switch (event->key_symbol)
    ...
    case DIKS_ESCAPE:       *key = KBD_UP;       break;
    ...
    case DIKS_RED:          *key = KBD_F1;        break;
