...
namespace io {
	class CodeMap {
		...
		public:
			int getCode(std::string codeStr);
			std::string getValue(int value);
			static CodeMap* getInstance();
			std::map<std::string, int>* cloneKeyMap();

			static void valuesToKeys (const std::set<int> &keys, std::set< ::util::key::type > &result);
	};
}
}
}
