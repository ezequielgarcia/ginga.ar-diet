...
IDirectFBDisplayLayer* DFBDeviceScreen::gfxLayer = NULL;
	...
	DFBDeviceScreen::DFBDeviceScreen(int numArgs, char* args[]) {
		...
		if (DFBDeviceScreen::dfb == NULL) {
			...
			ret = gfxLayer->SetBackgroundColor(gfxLayer, DFB_BG_R, DFB_BG_G, DFB_BG_B, DFB_BG_A);

			if (ret) {
				DirectFBError("DFBDeviceScreen set background color level error: ",ret);
			}
	...
	DFBDeviceScreen::~DFBDeviceScreen() {
		...
		cout << "[DFBDeviceScreen] windowPool size = " << windowPool->size() << endl;
		cout << "[DFBDeviceScreen] surfacePool size = " << surfacePool->size() << endl;
		...
		if (numOfDFBScreens == 0) {
			...
			gfxLayer = NULL;
			dfb = NULL;
			cout<<"[DFBDeviceScreen] DFB released."<<endl;
		}else{
			cout<<"[DFBDeviceScreen] WARNING: DFB could not be released."<<endl;
		}
	}
	...
	void DFBDeviceScreen::releaseSurface(void* sur) {
		...
		s->Clear(s, DFB_BG_R, DFB_BG_G, DFB_BG_B, DFB_BG_A);
		s->Release(s);
		s = NULL;
		sur = NULL;
	}
