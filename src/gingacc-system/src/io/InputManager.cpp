...
#include <connector/connector/handler/keyeventhandler.h>
...
namespace io {
	...
	void InputManager::initializeInputIntervalTime() {
		...
		cout << "[InputManager] imperative interval time:" << imperativeIntervalTime << "." << endl;
		cout << "[InputManager] declarative interval time:" << declarativeIntervalTime << "." << endl;
	}
	...
	void InputManager::release() {
		...
		if (actionsToInpListeners != NULL) {
			vector<LockedAction*>::iterator actionsIt;
			for(actionsIt = actionsToInpListeners->begin(); actionsIt != actionsToInpListeners->end(); actionsIt++){
				if ((*actionsIt)->events != NULL){
					delete (*actionsIt)->events;
					(*actionsIt)->events = NULL;
				}
			}
		...
		if (globalKeys.size()) {
			std::cout << "[InputManager] " << globalKeys.size() << " keys still registered, check your code! " << std::endl;
		}

		delete keyHandler;

		unlock();
		std::cout << "[InputManager] released" << std::endl;
	}
	...
	void InputManager::setConnector(connector::Connector *c) 
	{
		connector = c;
		keyHandler = new connector::KeyEventHandler();
	}

	connector::Connector*  InputManager::getConnector(){
		return connector;
	}
	...
	void InputManager::addInputEventListener(
		...
		if (!evs) { // Listen for NULL ??
			// The listener should be removed...
			return removeInputEventListener(listener);
		}
		
		if (evs->size() == 0) {  // Listen for empty set ??
			// The listener should be removed
			delete evs;
		    	return removeInputEventListener(listener);
		}
		...
		if (notifying) {
			...
			if (i != eventListeners->end()) {
				iEvs = i->second;
				if (compareSets(iEvs,evs)) { // are the keys we are listening are the same we were listening??
				    unlock();           // Do nothing
				    evs->clear();
				    delete evs;
				    return ;
			...
			// We have to register some new keys + maybe remove others.
			// Right now, we are releasing all the previous keys and reserving the new ones.
			unlock();
			removeInputEventListener(listener, false); // See if we have to release the previous keys
			lock();
			performInputLockedActions();
			(*eventListeners)[listener] = evs;

			if (keyHandler)
				reportKeysToConnector(true, evs);
			unlock();
		}
	}
	...
	void InputManager::removeInputEventListener(
		    ...
		    return removeInputEventListener(listener, false);
	}

	void InputManager::removeInputEventListener(
		    IInputEventListener* listener, bool report) {
		...
		if (notifying) {
			...
			i = eventListeners->find(listener);
			if (i != eventListeners->end()) {
				if (report && keyHandler)
					reportKeysToConnector(false, i->second);
				if (i->second != NULL) {
					delete i->second;
				}
	...
	void InputManager::reportKeysToConnector(bool isReserve, std::set<int> *set)
	{
		std::set<int> reportKeys;
		...
		std::set<int>::iterator k = set->begin();
		std::multiset<int>::iterator gk;

		while (k != set->end()) {
			int key = *k;
			gk = globalKeys.find(key);
			
			if (isReserve) {
				globalKeys.insert(key);
				modified |= (gk == globalKeys.end());
			} else {
				if (gk != globalKeys.end()) {
					globalKeys.erase(gk);
					modified |= globalKeys.find(key) == globalKeys.end();
				}
			}
			++k;
		}
		std::cout << "[InputManager] Sending new Key set to zapper" << std::endl;
		if (modified) {
			gk = globalKeys.begin();
			while (gk != globalKeys.end()) {
				reportKeys.insert(*gk);
				++gk;
			}
			std::set< ::util::key::type > rks;
			CodeMap::valuesToKeys(reportKeys, rks);
			keyHandler->send(connector, rks);
		}
	}
