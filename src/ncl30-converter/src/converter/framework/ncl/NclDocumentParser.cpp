...
namespace ncl {
	...
	void* NclDocumentParser::parseRootElement(DOMElement* rootElement) {
		...
		} else if(tagName=="smil") {
			wclog << "NclDocumentParser::parseRootElement SMIL!" << endl;
			SmilNcmDocumentConverter *smilTiny = new SmilNcmDocumentConverter(
					deviceLayout);

			void *ret = smilTiny->parseRootElement(rootElement);
			NclDocument *doc = (NclDocument *)
								smilTiny->getObject("return", "document");
			if(doc==NULL)
				wclog <<
				"NclDocumentParser::parseRootElement Warning! smilDoc NULL"
					  << endl;

			addObject("return", "document",
					  smilTiny->getObject("return", "document") );
			return ret;
		}
		else {
			wclog << "NclDocumentParser::parseRootElement Warning!";
